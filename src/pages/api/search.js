...
export default async function handler(req, res) {
	...
	try {
		const response = await axios.get(baseUrl + `v1/breeds/search?limit=10&page=0&q=${req.query.term}`, {
			headers: {
		...
		const getImagesForBreeds = (breedImageIds) => {
			...
			return breedImageIds.map((imageId) => axios.get(baseUrl + `v1/images/${imageId}`));
		};

		// get only the image Ids
		const imageIds = firstThreeBreeds.filter(b => b.reference_image_id).map((b) => b.reference_image_id);
